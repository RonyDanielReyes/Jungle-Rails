  ...
  describe "Validations" do
    describe "All Present" do
      it "will save successfully when all fields are present & passwords match" do
        @user = User.new(:first_name => 'Rony', :last_name => 'Reyes', :email => 'a@a.com', :password => 'test123', :password_confirmation => 'test123')
        expect(@user.save)
      end

      it "will throw an error when passwords do not match" do
        @user = User.new(:first_name => 'Rony', :last_name => 'Reyes', :email => 'a@a.com', :password => 'test123', :password_confirmation => 'test456')
        @user.save
    ...
    describe "Password Empty" do
      it "will throw an error when password is empty" do
        @user = User.new(:first_name => 'Rony', :last_name => 'Reyes', :email => 'a@a.com', :password_confirmation => 'test456')
        @user.save
        expect(@user.errors.full_messages).not_to be_empty
      end

      it "will throw an error when password confirmation is empty" do
        @user = User.new(:first_name => 'Rony', :last_name => 'Reyes', :email => 'a@a.com', :password => 'test456')
        @user.save
        expect(@user.errors.full_messages).not_to be_empty
      end
    end
    ...
    describe "Name Empty" do
      it "will throw an error when first name is empty" do
        @user = User.new(:last_name => 'Reyes', :email => 'a@a.com', :password => 'test123', :password_confirmation => 'test123')
        @user.save
        expect(@user.errors.full_messages).not_to be_empty
      end

      it "will throw an error when last name is empty" do
        @user = User.new(:first_name => 'Rony', :email => 'a@a.com', :password => 'test123', :password_confirmation => 'test123')
        @user.save
        expect(@user.errors.full_messages).not_to be_empty
      end
    end
    ...
    describe "Email Empty" do
      it "will throw an error when email is empty" do
        @user = User.new(:first_name => 'Rony', :last_name => 'Reyes', :password => 'test123', :password_confirmation => 'test123')
        @user.save
        expect(@user.errors.full_messages).not_to be_empty
      end
    end
    ...
    describe "Unique Email" do
      it "will throw an error when attempt to save a duplicate email (case insensitve)" do
        @user1 = User.new(:first_name => 'Rony', :last_name => 'Reyes', :email => 'a@a.com', :password => 'test123', :password_confirmation => 'test123')
    ...
    describe "Password Minimum Length" do
      it "will throw an error when password is less than 5 characters" do
        @user = User.new(:first_name => 'Rony', :last_name => 'Reyes', :email => 'a@a.com', :password => 'test', :password_confirmation => 'test')
        @user.save
        expect(@user.errors.full_messages).not_to be_empty
      end
    end
